...
class TopoSort {
  ...
  add(dependency, newDependents) {
    // cast to array
    newDependents = Array.isArray(newDependents) ? newDependents : [newDependents]; // get the existing dependents
  ...
  sort() {
    ...
    const l = []; // Find all the initial 0 incoming edge nodes. If not found, this is is a circular graph, cannot be sorted.
    ...
    nodesWithDependencies.forEach(nodeHasDependency => initialNodes.delete(nodeHasDependency)); // List of nodes with no unsorted dependencies

    const s = [...initialNodes];
    const allNodes = new Set(nodesWithDependents.concat(nodesWithDependencies)); // number of unsorted nodes. If it is not zero at the end, this graph is a circular graph and cannot be sorted.
    ...
    while (s.length !== 0) {
      const n = s.shift();
      l.push(n); // decrease unsorted count, node n has been sorted.

      --unsorted; // n node might have no dependency, so have to check it.
